...
//    };
    ...
    private void secondaryUpload() {
        DatabaseHelper db = DatabaseHelper.getInstance(this);
        if (db.countLocations() > Constants.CONFIG.MAX_LOCATIONS) {
            Log.i(TAG, "FIXME XXX SECONDARY UPLOAD! XXX FIXME");
            uploadData();
        }
    }
...
//        });
        ...
        tvCounter.setText(getString(R.string.empty));
...
//        05-30 04:19:08.988 ? W/ActivityManager: Unable to start service Intent { cmp=de.uniko.SoMA/.AlarmReceiver } U=0: not found
...
//    }
    ...
    private void updateCounter(long locationCount) {
        DatabaseHelper db = DatabaseHelper.getInstance(this);
        tvCounter.setText(String.format("%d", locationCount));
        tvCounter.setAlpha(1.0f);
        tvCounter.animate().alpha(0.2f).setDuration(600);
    }
...
//    }
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            switch (intent.getAction()) {
                case Constants.EVENT.GOOGLE_API_CLIENT_CONNECTED:
                    ...
                    break;
                case Constants.EVENT.FUSED_LOCATION_API_DISCONNECTED:
                    onOffSwitch.setChecked(false); // FIXME
                    showSnackBar("Disconnected");
                    break;
                case Constants.EVENT.UPLOAD_SUCCESS:
                    showSnackBar("Upload OK");
                    break;
            }
        }
    }
    ...
    private void registerBroadcastReceiver() {
        ...
        registerReceiver(mapsReceiver, new IntentFilter(Constants.EVENT.FUSED_LOCATION_API_DISCONNECTED));
        registerReceiver(mapsReceiver, new IntentFilter(Constants.EVENT.UPLOAD_SUCCESS));
